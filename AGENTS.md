...
- Do not enable retention locks, cross-region copies, higher volume
  performance, paid load balancers, or other paid features.
...
- Creating or deleting an instance, volume, image, backup, public IP, or DNS
  record.
- Formatting a disk, changing a partition table, changing a filesystem UUID,
  or deleting data.
...
- Use the normal non-root SSH account and passwordless `sudo` only for the
...
- After verifying the current backup-object limit, keep one accepted pair and
  at least two free object slots before starting a rotation. The 2026-09-03
