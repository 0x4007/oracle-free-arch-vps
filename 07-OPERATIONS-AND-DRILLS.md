...
quiesce applications
  ...
  -> confirm SSH and services are gone
  -> poll OCI for STOPPED with a fixed deadline
       -> STOPPED: continue
       -> still RUNNING: HARD_STOP_REQUIRED and stop
```

Set `action` to `wait-for-stopped` to use the bounded poller. The tool never
invokes `STOP`, `SOFTSTOP`, or `RESET`. A hard OCI `STOP` needs a separate exact
approval after the guest is proved quiescent.
