...
1. Confirm through the serial console or guest evidence that shutdown finished.
2. Refresh the instance lifecycle state and wait for normal convergence.
3. Check for an active OCI writer that may be changing the instance.
4. Stop and request approval before a hard `STOP` action.

Do not substitute `RESET`, `SOFTSTOP`, or process termination. After any hard
stop, treat the next boot and filesystem checks as mandatory recovery checks.
