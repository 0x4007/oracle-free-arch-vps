...
4. Quiesce stateful applications and run `sync` while the guest remains running.
5. Obtain exact approval and use OCI `SOFTSTOP` from the operator system. Wait
   up to 1,200 seconds for OCI `STOPPED` as described in
   `07-OPERATIONS-AND-DRILLS.md`.
6. On timeout or error, reread OCI state and stop. Never automatically issue
   immediate `STOP` or `RESET`. Immediate `STOP` needs separate approval and
   independent proof that the guest is already quiescent.
7. Start the same instance.
8. Verify the expected host key before accepting SSH.
9. Prove Arch, AArch64, root UUID, mounts, networking, SSH, services, and staged
   boot parity.
