...
async function waitForStopped(
  ...
  while (Date.now() < deadline) {
    ...
    if (stringField(instance, "lifecycle-state") === "STOPPED") {
  ...
  console.log(JSON.stringify({
    ...
    instruction:
      "Confirm that the guest is quiescent and obtain exact approval. This tool never invokes STOP, SOFTSTOP, or RESET.",
  }));
  Deno.exitCode = 3;
}
